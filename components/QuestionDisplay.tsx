...
const CrossIcon: React.FC<{ className?: string }> = ({ className = "h-5 w-5" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
...
const QuestionDisplay: React.FC<QuestionDisplayProps> = ({
  ...
  const getButtonClasses = useCallback((option: string): string => {
    ...
    if (showFeedback) {
      ...
      return `${baseDuoStyle} bg-transparent text-slate-100 border-2 border-slate-700 border-b-[4px] active:border-b-2 active:translate-y-[2px] cursor-pointer`;
    }
  ...
  return (
    <div className="space-y-6 fade-in pt-2 sm:pt-4 flex flex-col flex-grow">
      <div className="text-center text-sm text-slate-400 mb-3 sm:mb-4">
        سوال {toPersianDigits(currentQuestionNumber)} از {toPersianDigits(totalQuestions)}
      </div>
      <h2 
      ...
      >
        {options.map((option, index) => {
          ...
          const isSelectedIncorrectChoice = showFeedback && option === selectedAnswer && option !== question.answer;
          ...
          return (
            ...
            >
              <span className="flex-1 text-right font-medium">{option}</span>
              {isCorrectChoice && <CheckIcon className="h-5 w-5 sm:h-6 sm:w-6 text-white ms-2 me-1" />} 
              {isSelectedIncorrectChoice && <CrossIcon className="h-5 w-5 sm:h-6 sm:w-6 text-white ms-2 me-1" />}
            </button>
          );
        })}
      </div>
